...
| Before commit                               | `pnpm test`                                                                                  | Unit + Nuxt (both with coverage, gated by the thresholds in `vitest.shared.ts`) + local-auth + integration, then the four local browser gates (still images, silent video, fonts, Animation Effect rendering). Needs an installed Chrome/Chromium. Never run integration passes concurrently — serialise them (`docs/agents/parallel-rounds.md`).                           |
...
| Touching an Animation Effect shader         | `pnpm test:browser:animation-effects`                                                        | Mounts every catalogue effect at defaults and named range extremes in unattended Chromium on SwiftShader, then requires it to compile, light the frame without washing it out, and animate. Part of `pnpm test`; run it alone while editing a shader. Local only—it bundles this checkout's own effect modules.                                                             |
