...
export function useRequestFeedback() {
	 ...
	 * (`screens/[screenId].vue:209` → the asset-capability route →
	 * `server/modules/screen-output-assets/runtime.ts:27`) raises the first family — and
	 * before #286 its prose survived only because that call site supplies static wording
	 * and discards this function's answer. Widening a call site to
	 * `error: ({ message }) => …` is therefore now the improvement it looks like, which it
	 * was not between #271 and #286.
	 */
