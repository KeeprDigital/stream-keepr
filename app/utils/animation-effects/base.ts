...
// const DEBUGMODE = window.location.toString().indexOf('ANIMATION_EFFECTSDEBUG') !== -1

const win = typeof window == 'object'
let THREE = (win && window.THREE) || {}
if (win && !window.ANIMATION_EFFECTS) window.ANIMATION_EFFECTS = {}
const ANIMATION_EFFECTS = (win && window.ANIMATION_EFFECTS) || {}
ANIMATION_EFFECTS.register = (name, Effect) => {
  return ANIMATION_EFFECTS[name] = (opts) => new Effect(opts)
}
ANIMATION_EFFECTS.version = '0.5.24'

export {ANIMATION_EFFECTS}
...
const error = function() {
  Array.prototype.unshift.call(arguments, '[ANIMATION_EFFECTS]')
  return console.error.apply(this, arguments)
}

ANIMATION_EFFECTS.AnimationBase = class AnimationBase {
  constructor(userOptions = {}) {
    if (!win) return false
    ANIMATION_EFFECTS.current = this
    ...
    try {
      ...
      if (this.options.backgroundColor) {
        console.log('[ANIMATION_EFFECTS] Falling back to backgroundColor')
  ...
  initThree() {
    if (!THREE.WebGLRenderer) {
      console.warn("[ANIMATION_EFFECTS] No THREE defined on window")
      return
    }
    // Set renderer
  ...
  setInteractionPoint(xNorm = 0.5, yNorm = 0.5) {
    const x = Math.max(0, Math.min(1, xNorm)) * this.width
    const y = Math.max(0, Math.min(1, yNorm)) * this.height
    this.setInteractionPixels(x, y)
  }

  setInteractionPixels(x, y) {
    this.mouseX = x
    this.mouseY = y
    if (this.uniforms) {
  ...
  destroy() {
    ...
    if (ANIMATION_EFFECTS.current === this) {
      ANIMATION_EFFECTS.current = null
    }
  }
}

export default ANIMATION_EFFECTS.AnimationBase