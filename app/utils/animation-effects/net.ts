 ...
 *
 * The line stringing, its vertex-colour restoration, and the exact buffer
 * bound live in `connectionLines.ts`, shared with globe. Two inert fork
 * behaviours drop: the pointer-proximity point scaling (its raycaster was
 * never constructed, so scale held 1) and the mobile field reduction (a Screen
 * Output is never a mobile browser).
 */
...
export function createNetDelegate(): SceneEffectDelegate<NetAnimationParams> {
	...
	function buildField(three: typeof THREE) {
		...
		lines = createConnectionLineField(three, points.length, current);
