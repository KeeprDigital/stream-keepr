...
- An intent that takes over a phase still in flight carries **both** concurrent schedules, not just its own axis. A graphic that never left program is still crossing whatever it was crossing and still cycling wherever it had reached, so a Take reversing an exit keeps both — dropping either would put that channel back at the Graphic Resting State in the very frame the enter/exit channel was made smooth. An Out is the one intent this is not the last word for, and only about the update: it may carry a transition that never began, so the exit path decides afterwards whether an exit has one to run underneath. Cycling needs no such adjudication — an origin still in the future simply projects the Graphic Resting State — so that path refreshes it and never strips it.
