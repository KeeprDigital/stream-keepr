...
- `MAX_REALTIME_MESSAGE_BYTES` is this account's **confirmed ceiling**, not merely the documented floor it was chosen as. When this decision was taken, nothing in the repository recorded the plan — #95 said the same — so 64 KiB was adopted as the floor Ably documents for Free and Standard, on the reasoning that measuring against a floor can only cost a reload that was not needed, where measuring against a ceiling nobody established would cost a notification that never arrives. The account owner has since confirmed the package is **Free** (#189, recorded beside the constant in `shared/types/messages.ts`), whose per-message limit is that same 64 KiB. Every boundary measured above is therefore exact rather than conservative, and the reasoning that chose the floor is left standing because it is what should be re-applied if the package ever changes.
