...
- **The lease holder stays browser-scoped.** Leases guard concurrent _editors_, and one person in two browsers is two editors — a userId holder would let them silently corrupt each other. `holderSessionId` carries the Better Auth **session id** (the same granularity as today's authorId); takeover UI resolves session → user for display.
...
- `events/[id]/screens/slug/[slug].get` **leaves** the public surface: it folds into the capability surface, requiring the Screen Output Asset Capability bearer the output page already holds in its URL hash, and refuses 404 like the rest of that surface.
  - **Amended on implementation (#397): it accepts a session as well as that bearer.** This bullet said the bearer and nothing else, and implementing it that way would have broken the editor's own previews. `PreviewOutputAside` and `Graphics/Compositor/Preview` open the same page with `embed=preview` and pass **no** capability, deliberately — a preview "resolves media as the author rather than through a Screen Output Asset Capability" — so the route they bootstrap from cannot demand one. The alternatives were both worse: minting capabilities for previews contradicts that preview design, and a second session-gated route for the same resource means a repository choosing a path by credential. The session arm adds no reach — every signed-in user can already read the same Screen from `/api/events/:id/screens/**`, and `mapScreenToResponse` strips the capability digest either way. The route lives at `/api/screen-output/events/:id/screens/slug/:slug`, so its membership of the exempt surface is structural rather than a pattern in the allowlist.
- **Realtime token: dual-grant, no anonymous issuance.** An authenticated user gets today's grant shape (event channel plus all screen channels, subscribe/history/presence) with `clientId` = the userId, not `'*'`. A capability bearer gets a grant narrowed to that screen's channels plus its event channel, `clientId` = a screen-output identity. Screen outputs stay credential-free in ADR-0008's sense — the capability already in hand _is_ their credential.
