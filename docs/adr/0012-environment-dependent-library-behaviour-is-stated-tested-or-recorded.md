...
captures `NODE_ENV` into a module-level constant at import (`env-impl.mjs`).
`isTest()` additionally reads the live `TEST` variable — a name that can only
add test-ness, never remove it — so nothing constructed in-process can un-see
the environment vitest started under.
...
| `cookies/index.mjs` — secure-cookie prefix falls back to `isProduction` when `useSecureCookies` is unstated, protocol is `'auto'` and there is no string `baseURL` (two sites in the dist: the cookie getter and the standalone cookie utilities) | Session cookies unprefixed locally, `__Secure-`-prefixed in production                                                     | **Accepted blindness** — see below                                                                                                                                                                                       |
