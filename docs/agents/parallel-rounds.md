...
- **Eight concurrent lanes exceed this machine.** Load passed 120, `sysmond` died (so `pgrep`/`pkill` failed with "Cannot get process list" — and a broken `pgrep` piped to `wc -l` reads as a reassuring zero), LaunchServices stopped resolving apps, and the 1Password agent crash-restarted with its socket dead. Cap rounds at five or six lanes, and treat full-repo eslint/typecheck as heavy: six concurrent `eslint .` runs were the proximate overload. Under a hold, single-file test runs and file writes are fine; suites, builds, lints, and typechecks are not.
- **Serialize signing with a lock, and background nothing that signs.** A `mkdir`-based lock in the scratchpad (acquire → ONE foreground attempt → release in the same chain, steal only if lock is >5 min old with no live signer) ended the wedge cycle. A harness timeout that kills a commit mid-sign orphans an `op-ssh-sign` and leaves work staged-but-uncommitted.
...
- **`git worktree add` ships neither `node_modules` nor `.nuxt`.** All six lanes started without them; `pnpm install --frozen-lockfile` (whose postinstall `nuxt prepare` generates `.nuxt`) is a heavy run — serialize it with the suites.
- **A machine-wide mkdir suite lock kept six lanes from stacking heavy runs — but it has no fairness.** Every waiter races `mkdir` after its own sleep, so a given waiter can lose repeatedly; one lane's queued job sat 8+ minutes while the lock changed hands three times. Long waits are contention, not a wedge; check the owner file before assuming. If starvation matters, hand the queued work to whoever next holds the tree (the round did exactly this: the reviewer ran the implementer's queued proof).
- **The background wrapper's exit notice lied at least four more times this round, always reassuringly.** One lane's `SUITE_EXIT=0` sat three lines below the `LINT_EXIT=1` that was the truth. Wrap the runner directly so the lock script's exit is the runner's own, and read the log's per-step markers, never the notice.
...
- **An implementer with a queued locked job must not mutate until that job has run or been cancelled.** A lane queued its full suite on the lock and then started mutation rows; had the lock freed mid-row, its own authoritative run would have executed against a mutated tree — the two-agents-one-worktree hazard with one agent. Cancel the queued job first (single-file rows are lock-exempt), then re-queue.
...
- **The Bash tool SIGTERMs foreground commands at ten minutes, which reaps lock-wait loops.** A lock-wait written with a longer timeout died at 10:00 exit 143; the fix is backgrounding the whole acquire-run-release chain — and checking afterwards that the kill did not orphan a lock you owned. The mkdir suite lock's unfairness also compounded: 20-second sleepers repeatedly lost handovers to 3-second pollers (~9 minutes across 17 waits in one lane); poll fast or hand queued work to whoever holds the tree.
