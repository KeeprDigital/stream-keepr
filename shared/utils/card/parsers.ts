...
import { defaultMtgCardData } from './default';

export function cardParser(card: ScryfallCard.Any): MtgCard {
	const cardData = defu({
		id: card.id,
		name: card.name,
		set: card.set_name,
	...
	switch (card.layout) {
		...
		case 'split': {
			const isAftermath = card.keywords?.includes('Aftermath');
			return defu({
				imageData: {
					front: card.image_uris ?? null,
					back: null,
